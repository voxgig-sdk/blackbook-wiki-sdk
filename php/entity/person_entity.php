...
class PersonEntity
{
    ...
    {
        if ($args) {
            $this->_data = BlackbookWikiHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = BlackbookWikiHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws BlackbookWikiError on failure (item-5 convention).
     */
