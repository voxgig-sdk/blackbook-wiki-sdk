...
// BlackbookWiki SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class BlackbookWikiTestFeature extends BlackbookWikiBaseFeature
{
    ...
    public function init(BlackbookWikiContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BlackbookWikiContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BlackbookWikiParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BlackbookWikiContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BlackbookWikiContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BlackbookWikiParam::call($ctx, $k);
