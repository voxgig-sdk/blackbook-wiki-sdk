...
// BlackbookWiki SDK retry feature
...
class BlackbookWikiRetryFeature extends BlackbookWikiBaseFeature
{
    ...
    public function init(BlackbookWikiContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BlackbookWikiContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BlackbookWikiContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BlackbookWikiContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
