...
class BlackbookWikiMakeError
{
    public static function call(?BlackbookWikiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BlackbookWikiContext([], null);
        }
        $op = $ctx->op ?? new BlackbookWikiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BlackbookWikiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BlackbookWikiError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BlackbookWikiError('', $msg, $ctx);
        ...
        if ($err instanceof BlackbookWikiError) {
