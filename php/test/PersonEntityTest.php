...
class PersonEntityTest extends TestCase
{
    ...
    {
        $testsdk = BlackbookWikiSDK::test(null, null);
    ...
    public function test_stream(): void
    {
        ...
        $base = BlackbookWikiSDK::test($seed, null);
        $seen = iterator_to_array($base->Person(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = BlackbookWikiConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = BlackbookWikiSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLACKBOOKWIKI_TEST_PERSON_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function person_basic_setup($extra)
{
    ...
    $client = BlackbookWikiSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("BLACKBOOKWIKI_TEST_PERSON_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "BLACKBOOKWIKI_TEST_PERSON_ENTID" => $idmap,
        "BLACKBOOKWIKI_TEST_LIVE" => "FALSE",
        "BLACKBOOKWIKI_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["BLACKBOOKWIKI_TEST_PERSON_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["BLACKBOOKWIKI_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new BlackbookWikiSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["BLACKBOOKWIKI_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
