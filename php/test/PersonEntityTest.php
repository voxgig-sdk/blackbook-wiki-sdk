...
class PersonEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $cfg = BlackbookWikiConfig::shared_config();
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLACKBOOK_WIKI_TEST_PERSON_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function person_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BLACKBOOK_WIKI_TEST_PERSON_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["BLACKBOOK_WIKI_TEST_PERSON_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["BLACKBOOK_WIKI_TEST_LIVE"] === "TRUE") {
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["BLACKBOOK_WIKI_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
