...
// BlackbookWiki SDK feature test
//
...
class FtCtrl extends BlackbookWikiControl
{
...
class FtClient
{
    ...
    private BlackbookWikiUtility $utility;

    public function __construct(BlackbookWikiUtility $utility, array $options)
    {
    ...
    public function get_utility(): BlackbookWikiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BlackbookWikiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BlackbookWikiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BlackbookWikiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BlackbookWikiConfig::make_feature($name);
    ...
    private static function buildUrl(BlackbookWikiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BlackbookWikiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BlackbookWikiResponse($fetched);
        }
    ...
    private function fail(BlackbookWikiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BlackbookWikiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
