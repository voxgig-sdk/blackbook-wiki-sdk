...
[MCP](https://modelcontextprotocol.io) server exposing the BlackbookWiki SDK as
two agent tools — `blackbook-wiki_list` and `blackbook-wiki_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/blackbook-wiki-mcp)
make build

# 2. Provide credentials via the environment
export BLACKBOOK_WIKI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user blackbook-wiki \
  -- /absolute/path/to/blackbook-wiki-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./blackbook-wiki-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// blackbook-wiki_list: first page of records
{ "entity": "person" }
{ "entity": "person", "query": { } }

// blackbook-wiki_load: one record by id
{ "entity": "person", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/blackbook-wiki-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user blackbook-wiki \
     -- "$PWD"/dist/*/blackbook-wiki-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `blackbook-wiki_list` and `blackbook-wiki_load` tools now appear
   in new sessions. Ask the agent to *"list person using blackbook-wiki"*
   and it calls `blackbook-wiki_list` with `{"entity":"person"}`.
...
```sh
export BLACKBOOK_WIKI_APIKEY=sk_live_xxx            # API key
export BLACKBOOK_WIKI_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `blackbook-wiki_list` tool
...
### Call the `blackbook-wiki_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `blackbook-wiki_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `blackbook-wiki_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BLACKBOOK_WIKI_APIKEY` | API key sent with every request. |
| `BLACKBOOK_WIKI_BASE` | Optional override of the API base URL. |
...
```sh
./blackbook-wiki-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"blackbook-wiki_load","arguments":{"entity":"person","query":{"id":1}}}}'
```
