...
//
// NewBlackbookWikiSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.BlackbookWikiSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewBlackbookWikiSDK(opts)
}
...
func fcCandidates(client *sdk.BlackbookWikiSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.BlackbookWikiSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.BlackbookWikiSDK, name string) any {
