...
import (
	...
	omni "github.com/voxgig-sdk/blackbook-wiki-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BlackbookWikiError); is {
				out := map[string]any{
					"name":    "BlackbookWikiError",
...
func omniCtx(arg any, client *sdk.BlackbookWikiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BlackbookWikiSDK); is {
			client = live
