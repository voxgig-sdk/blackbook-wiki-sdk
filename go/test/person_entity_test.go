...
func TestPersonEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLACKBOOKWIKI_TEST_PERSON_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func personBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLACKBOOKWIKI_TEST_PERSON_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLACKBOOKWIKI_TEST_PERSON_ENTID"])
	...
	if env["BLACKBOOKWIKI_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewBlackbookWikiSDK(core.ToMapAny(mergedOpts))
	}
