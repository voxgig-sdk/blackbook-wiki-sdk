...
const __1 = require("..");
const SDK_NAME = 'BlackbookWikiSDK';
...
const TEST_SEED = { "entity": { "person": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BlackbookWikiSDK\b/.test(code) ||
        /\bBlackbookWikiSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BlackbookWikiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BlackbookWikiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
