...
module BlackbookWikiUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = BlackbookWikiContext.new({}, nil)
    end
    op = ctx.op || BlackbookWikiOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || BlackbookWikiResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(BlackbookWikiError) ? err.msg : err.to_s
    ...
    sdk_err = BlackbookWikiError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(BlackbookWikiError)
