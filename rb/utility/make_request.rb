...
module BlackbookWikiUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BlackbookWikiResponse.new({})
    ...
    if fetch_err
      ...
      response = BlackbookWikiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BlackbookWikiResponse.new(fetched)
    else
