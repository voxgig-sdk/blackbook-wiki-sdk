...
require_relative 'BlackbookWiki_types'


class BlackbookWikiSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BlackbookWikiHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BlackbookWikiHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BlackbookWikiFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BlackbookWikiUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BlackbookWikiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BlackbookWikiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BlackbookWikiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => BlackbookWikiError.new(
        "#{op}_allow",
        "BlackbookWikiSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = BlackbookWikiHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BlackbookWikiError.new(
        "graphql_error", "BlackbookWikiSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = BlackbookWikiSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
