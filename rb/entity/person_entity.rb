# BlackbookWiki SDK Person entity
...
class PersonEntity
  ...
  def data_set(args)
    if args
      @_data = BlackbookWikiHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = BlackbookWikiHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Person>, Array] the matching Person items; raises BlackbookWikiError on failure
