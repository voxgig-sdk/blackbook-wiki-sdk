# BlackbookWiki SDK test runner

from __future__ import annotations
import os
import json

from blackbookwiki_sdk.utility.voxgig_struct import voxgig_struct as vs


class BlackbookWikiTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BlackbookWikiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BlackbookWikiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BlackbookWikiTestRunner.getenv("BLACKBOOK_WIKI_TEST_LIVE")
        override = BlackbookWikiTestRunner.getenv("BLACKBOOK_WIKI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BlackbookWikiTestRunner.getenv(key)
        ...
        explain = BlackbookWikiTestRunner.getenv("BLACKBOOK_WIKI_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BlackbookWikiTestRunner._test_control is not None:
            return BlackbookWikiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BlackbookWikiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BlackbookWikiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BlackbookWikiTestRunner.load_test_control()
...
def load_env_local():
    BlackbookWikiTestRunner.load_env_local()


def env_override(m):
    return BlackbookWikiTestRunner.env_override(m)


def entity_data(v):
    return BlackbookWikiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BlackbookWikiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BlackbookWikiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BlackbookWikiTestRunner.load_test_control()


def live_client_options():
    return BlackbookWikiTestRunner.live_client_options()


def live_delay_ms():
    return BlackbookWikiTestRunner.live_delay_ms()
