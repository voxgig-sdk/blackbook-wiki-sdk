...
boru-driven command-line client **and** interactive REPL for the BlackbookWiki
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/blackbook-wiki-cli)
make build

# 2. See usage (words, entities, env vars)
./blackbook-wiki-cli --help

# 3. Provide credentials once, via the environment
export BLACKBOOK_WIKI_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./blackbook-wiki-cli list person

# 5. Override the API base URL for a single call
BLACKBOOK_WIKI_BASE=https://api.example.com ./blackbook-wiki-cli list person

# 6. No arguments -> interactive REPL
./blackbook-wiki-cli
blackbook-wiki> list person
blackbook-wiki> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/blackbook-wiki-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/blackbook-wiki-cli list person
   ```
...
Run with no arguments to open a REPL (prompt `blackbook-wiki>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `person`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/blackbook-wiki-cli`. |
...
(`./blackbook-wiki-cli <expr>`) and interactively (the REPL), and expressions compose the
